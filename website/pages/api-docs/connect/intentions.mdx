...
## Upsert Intention by Name <sup>Beta</sup>
...
`service-intentions` config entry directly for the destination.
...
- `Permissions` `(TODO)` - link to config-entry for this?
...
- `SourceName` `(string: <ignored>)` - The source of the intention.
  For a `SourceType` of `consul` this is the name of a Consul service. The
  service doesn't need to be registered.
  This field is *always* populated from a portion of the `source` url parameter.
  Any provided data is ignored.

- `SourceNS` `(string: <ignored>)` <EnterpriseAlert inline /> - The namespace for the
  `SourceName` parameter.
  This field is *always* populated from a portion of the `source` url parameter.
  Any provided data is ignored.

- `DestinationName` `(string: <ignored>)` - The destination of the intention.
  The intention destination is always a Consul service, unlike the source.
  The service doesn't need to be registered.
  This field is *always* populated from a portion of the `source` url parameter.
  Any provided data is ignored.

- `DestinationNS` `(string: <ignored>)` <EnterpriseAlert inline /> - The namespace for the
...
[upserting by name](#upsert-intention-by-name) or by editing the 
[`service-intentions`](/docs/agent/config-entries/service-intentions)
config entry kind directly for the destination..
...
## Read Specific Intention by Name <sup>Beta</sup>
...
## Delete Intention by Name <sup>Beta</sup>
...
-> **Deprecated** - This endpoint is deprecated in Consul 1.9.0 in favor of 
[deleting by name](#delete-intention-by-name) or by editing the 
[`service-intentions`](/docs/agent/config-entries/service-intentions)
config entry kind directly for the destination..
...
-> **Note:** This endpoint will always treat intentions with
`Permissions` defined as *deny* intentions during evaluation, as this
endpoint is only suited for networking layer 4 (e.g. TCP)
integration.

-> **Note:** This endpoint will always treat intentions with
`Permissions` defined as *deny* intentions during evaluation, as this 
endpoint is only suited for networking layer 4 (e.g. TCP) integration.
