...
1. There can only be one `ProxyDefaults` resource and it's name **must** be `global`.
...
### ServiceIntentions <sup>(1.9.0 beta)</sup>
...
1. The name of the service this intention is configuring is set in `spec.destination.name`.
