...
Services should periodically use the [intention match
API](/api/connect/intentions#list-matching-intentions) to retrieve all relevant
intentions with itself as a destination. These cached intentions should be
consulted for each incoming connection/request to determine if it should be
...
topologies and allowing connections between services in each datacenter without
