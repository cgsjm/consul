...
- **Authorization** - As a server accepting connections, verify the client
  ...
  - **Simple integration (TCP-only)** - Call the [authorizing
    ...
    -> **NOTE:** This API call is meant to be done in the connection path, so
    if the local Consul agent is down or unresponsive it will effect the
    success rate of new connections. The authorize API call should be called
    against the local Consul agent. The agent uses locally cached data to
