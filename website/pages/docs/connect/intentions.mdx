...
Intentions are enforced by the [proxy](/docs/connect/proxies) or [natively
integrated application](/docs/connect/native) on inbound connections or
requests.

You can define intentions to control Connect traffic authorization for both
networking layer 4 (e.g. TCP) and networking layer 7 (e.g. HTTP).
...
is "deny all", then all Connect connections or requests are denied by default.

## Intention Basics

Intentions can be managed primarily via
