...
provided by the [`/v1/agent/connect/ca/roots`](/api/agent/connect) endpoint. 
The connections (L4) or requests (L7) should be validated against the
intentions for the destination provided by the
[`/v1/connect/intentions/match?by=destination`](/api/connect/intentions#list-matching-intentions)
endpoint.

After validating the client certificate from the caller, the proxy should
consult the cached intentions for each incoming
connection/request to determine if it should be accepted or rejected.
...
The leaf cert, root cert, and intentions endpoints support blocking queries.
These should be used if possible to get near-immediate updates for root cert
rotations, leaf expiry, intention changes, etc.
